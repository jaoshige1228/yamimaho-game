...
フロントは **Vue 3 SPA（CSR）**、ゲームロジックと永続化は **Laravel JSON API** が担います。**SSR（サーバーサイド HTML 生成）は採用しません。**

| 層 | 技術 | 役割 |
|----|------|------|
| 表示 | Vue + Vite（`frontend/`） | ルーティング・UI・SE/BGM。`onMounted` / ルータガードで `/api` を fetch |
| 権威 | Laravel（`backend/routes/api.php`） | 戦闘・ダンジョン・所持品など **すべてのゲーム状態** |
| 配信 | S3 + CloudFront | 静的 `index.html` + JS/CSS。404/403 は SPA フォールバック |

ゲーム画面は SEO 不要・状態は API 依存・インフラは静的ホスティング前提のため、Nuxt / Inertia 等による SSR 化は行いません。将来 LP の SEO/OGP が必要になった場合も、トップだけ SSG するか LP を別サイトに分離し、ゲーム本体 SPA は現状維持が現実的です。
...
待機画面から **1 層** に入り、「進む」で深さを進めます（全 3 層構成、現状プレイ可能なのは 1 層のみ）。各層の最深部でボス戦があり、撃破で次の層が解放されます。戦闘（30%）と探索イベント（70%）の抽選、矢トラップ・宝箱などの詳細は [`docs/ダンジョン探索.md`](docs/ダンジョン探索.md) を参照してください。新規イベントの企画データ形式は [`docs/ダンジョンイベント企画テンプレ.md`](docs/ダンジョンイベント企画テンプレ.md) を参照してください。
